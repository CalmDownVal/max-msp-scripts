...
import { FREQ_MAX, FREQ_MIN, GAIN_MAX, GAIN_MIN, HANDLE_DIAMETER, HANDLE_RADIUS, RESO_MAX, RESO_MIN } from "./constants";
...
export const Convert = (() => {
	...
	const self = {
		...
		z2r(z: number) {
			return Math.pow(10.0, resoMinExp + clamp(z / MAX_Z) * (resoMaxExp - resoMinExp));
		},
		r2z(r: number) {
			return MAX_Z * (Math.log10(r) - resoMinExp) / (resoMaxExp - resoMinExp);
		},
	};
