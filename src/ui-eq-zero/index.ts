...
function paint() {
	...
	const head: Point = {
		x: 0.0,
		y: Convert.g2y(cascade.calculateGainAt(Convert.VISIBLE_FREQ_MIN)),
	};

	const tail: Point = {
		x: vw,
		y: Convert.g2y(cascade.calculateGainAt(Convert.VISIBLE_FREQ_MAX)),
	};
	...
	for (index = 0; index < count; index += 1) {
		...
		node = {
			x: Convert.f2x(filter.freq),
			y: Convert.g2y(cascade.calculateGainAt(filter.freq)),
			prev,
		};
	...
	for (index = 0; index < GRID_X_HIGHLIGHT.length; index += 1) {
		x = Convert.f2x(GRID_X_HIGHLIGHT[index]);
		mgraphics.move_to(x, 0.0);
		mgraphics.line_to(x, vh);
	}

	for (index = 0; index < GRID_Y_HIGHLIGHT.length; index += 1) {
		y = Convert.g2y(GRID_Y_HIGHLIGHT[index]);
		mgraphics.move_to(0.0, y);
		mgraphics.line_to(vw, y);
	}
...
function ondblclick(x: number, y: number) {
	...
	if (hit) {
		...
		const rx = x / mgraphics.size[0];
		if (rx < 0.1) {
			cascade.addFilter("hp2", Convert.x2f(x), Convert.y2r(y), undefined);
		}
		else if (rx < 0.25) {
			cascade.addFilter("ls2", Convert.x2f(x), undefined, Convert.y2g(y));
		}
		else if (rx > 0.9) {
			cascade.addFilter("lp2", Convert.x2f(x), Convert.y2r(y), undefined);
		}
		else if (rx > 0.75) {
			cascade.addFilter("hs2", Convert.x2f(x), undefined, Convert.y2g(y));
		}
		else {
			cascade.addFilter("pn2", Convert.x2f(x), undefined, Convert.y2g(y));
		}
	}
...
function onwheel(x: number, y: number, _dx: number, dy: number) {
	...
	const filter = cascade.getFilter(num);
	const flip = filter?.type === "pn2" ? Math.sign(filter.gain) : 1.0;

	cascade.moveFilterZ(Math.sign(dy) * flip * 1.0, num);
	mgraphics.redraw();
}
...
function subdivide(a: Point, b: Point) {
	...
	const x = (a.x + b.x) * 0.5;
	const y = Convert.g2y(cascade.calculateGainAt(Convert.x2f(x)));
	const node = {
		x,
		y,
		prev: a,
